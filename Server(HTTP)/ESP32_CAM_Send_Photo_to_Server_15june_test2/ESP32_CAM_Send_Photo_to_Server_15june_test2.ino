...
String serverName = "192.168.29.176";  //--> Change with your server computer's IP address or your Domain name.
...
void loop() {
  ...
  if (currentMillis - previousMillis >= Interval) {
    ...
    lox.begin(); // Initialize VL53L0X sensor
    ...
    if (measure.RangeStatus != 4) {  // phase failures have incorrect data
        ...
        if (distance < 100) {
            ...
            // Print current time, date, and day of the week
            printCurrentTime();
            printCurrentDate();
            printDayOfTheWeek();
            Serial.println("Sleeping for 5 minutes");
            delay(1000); // Delay for 1 second before sleeping
            esp_sleep_enable_timer_wakeup(1 * 60 * uS_TO_S_FACTOR); // 5 minutes
